...
- OpenCV + NumPy + scikit-image + skan — HSV-сегментация, скелетонизация, векторизация
...
│       ├── test_maps.pptx   # Тестовый PPTX с картами (2 слайда)
│       ├── test_1.jpg..test_3.jpg  # Тестовые JPG карт
...
│   ├── __init__.py
│   ├── map_pipeline.py      # ✅ Оркестратор (вызывает модули по цепочке)
│   ├── map_classifier.py    # ✅ Gemini → тип карты 1-13
│   ├── map_layer_splitter.py # ✅ PPTX → background + objects
│   ├── map_background.py    # ✅ Замена подложки (mapbox, keep; остальные TODO)
│   ├── map_objects_redesign.py # ⬜ СЛЕДУЮЩИЙ: пересадка объектов
│   ├── map_vectorizer.py    # ⬜ HSV + Skan (есть как vectorize_clean.py)
│   ├── map_assembler.py     # ⬜ Финальная сборка
│   └── vectorize_clean.py   # Рабочий прототип HSV+Skan (станет map_vectorizer.py)
...
6. map_vectorizer.py — HSV + Skan (для растровых маршрутов)
...
1. Gemini OCR → названия мест с растра (работает, 16 названий)
2. Gemini → грубый центр для viewbox (центр ок, масштаб нет)
3. Nominatim + viewbox (bounded=1) → точные GPS (БЕЗ viewbox точки улетают за 500км!)
4. BBox из точек + 5% padding → Mapbox Static API → лучший результат
5. Размер Mapbox = размер оригинальной подложки (НЕ хардкодить)
6. Объекты пересаживать по формуле: pixel_x = (lon-west)/(east-west)*w, pixel_y = (north-lat)/(north-south)*h (TODO)
